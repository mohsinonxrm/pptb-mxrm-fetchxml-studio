...
type BuilderAction =
	...
	| {
			type: "SYNC_LAYOUT_WITH_FETCHXML";
...
function builderReducer(state: BuilderState, action: BuilderAction): BuilderState {
	switch (action.type) {
		...
		case "SYNC_LAYOUT_WITH_FETCHXML": {
			...
			} else if (action.primaryIdAttribute) {
				// Generate new config from FetchXML using the real primary key from metadata
				newConfig = generateLayoutFromFetchXml(
					state.fetchQuery,
					action.primaryIdAttribute,
					action.attributeTypeMap,
				);
			} else {
				// No primary key available yet — skip until metadata is loaded
				return state;
			}
...
interface ViewLoadInfo {
	...
	entitySetName: string;
	name: string;
	/** The FetchXML string at the point of save/load — generated by the caller using real entity metadata */
	originalFetchXml: string;
}
...
export function BuilderProvider({ children }: { children: ReactNode }) {
	...
	const contextValue: BuilderContextValue = {
		...
		setLoadedView: (viewInfo: ViewLoadInfo) =>
			dispatch({
				...
				viewInfo: {
					id: viewInfo.id,
					type: viewInfo.type,
					// originalFetchXml is supplied by the caller, which has entity metadata available.
					// The store never derives/guesses the primary key itself.
		...
		syncLayoutWithFetchXml: (attributeTypeMap?: Map<string, string>, primaryIdAttribute?: string) =>
			dispatch({ type: "SYNC_LAYOUT_WITH_FETCHXML", attributeTypeMap, primaryIdAttribute }),
